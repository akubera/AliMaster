...
class Application:
    """
    Main application class which houses multiple threads for gui and server
    communication.
    """
    ...
    def __init__(self, *,
        ...
        if gui_thread is True:
            self.gui_thread = Thread(name='GuiThread',
                                     target=self.main_gui_loop)
    ...
    def get_new_window(self, title, minsize=(500, 300)):
        """
        Returns a new window with the application's tk root as the master. This
        window will keep the application alive if all other windows are closed.
        If this is the last window, this will close the application.
        """
